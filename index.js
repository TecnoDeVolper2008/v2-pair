...
const {  state, saveCreds } =await useMultiFileAuthState('./session/'+id)
   ...
   if (pairingCode && !sock.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number \nFor example: +94729787750 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')

         // Ask again when entering the wrong number
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +94729787750")))
...
//------------------------------------------------------
    ...
    if (connection == "open") {
      ...
      const ethix = await sock.sendMessage(sock.user.id, {
        text: `Kawdhitha-Alya&` + output.split('/')[3]
      })
      ...
      await sock.sendMessage(sock.user.id, { text: `> Queen Alya Powered By Kawdhitha nIrmal` }, { quoted: ethix });
      await delay(1000 * 2);
      process.exit(0);
    }
        if (
